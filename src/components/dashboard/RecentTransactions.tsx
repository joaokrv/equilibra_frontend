  ...
  <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 delay-300">
    <div className="glass p-4 sm:p-6 lg:p-8 rounded-2xl sm:rounded-3xl pb-6 sm:pb-10">
      <div className="flex items-center justify-between gap-3 mb-5 sm:mb-8">
        <h4 className="text-lg sm:text-xl font-bold text-foreground">{translate(language, 'topTransactions')}</h4>
        <Link 
          to="/extrato" 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3 sm:gap-5 lg:gap-6">
        {transacoesList.slice(0, 3).map((transacao) => (
          <div
            key={transacao.id}
            className="flex items-start justify-between gap-3 p-3 sm:p-5 bg-secondary/20 rounded-xl sm:rounded-2xl border border-foreground/5 hover:border-primary/30 transition-all cursor-pointer group"
          >
