      ...
      <div ref={dialogRef} className="glass w-full max-w-sm rounded-2xl sm:rounded-3xl p-5 sm:p-8 relative z-10 animate-in zoom-in-95 duration-200">
        ...
        <div className="flex flex-col sm:flex-row gap-3 mt-6">
          <button
            onClick={onCancel}
            disabled={isLoading}
            className="flex-1 py-2.5 rounded-xl border border-foreground/10 text-sm font-bold text-muted-foreground hover:bg-foreground/5 hover:text-foreground transition-all disabled:opacity-50"
          >
            {tr('CANCELAR', 'CANCEL')}
          </button>
          <button
